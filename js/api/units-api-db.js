...
(function () {
    ...
    const adapter = {
        listUnits: async (limit = 500) => {
            ...
            // try table candidates in order (katalog replaces master_unit)
            const candidates = ['unit', 'katalog', 'master_unit'];
        ...
        getUnitById: async (id) => {
            ...
            const candidates = ['unit', 'katalog', 'master_unit', 'units'];
            for (const t of candidates) {
                try {
                    if (typeof window.tableExists === 'function') {
                        const ok = await window.tableExists(t, sup);
        ...
        createUnit: async (payload) => {
            ...
            const candidates = ['unit', 'katalog', 'master_unit', 'units'];
            // prefer non-master for create; avoid creating katalog/master_unit from UI
            const insertOrder = candidates.filter(t => t !== 'master_unit' && t !== 'katalog').concat('master_unit');
            let lastErr = null;
