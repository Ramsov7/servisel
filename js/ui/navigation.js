    ...
    function updateHeaderSearchVisibility(activeSectionId) {
        ...
        try {
            ...
            if (activeSectionId === 'lainnya') {
                ...
                // Ensure the 'Lihat Katalog' header control is not present on Jasa page
                try {
                    const selectRowInner = document.querySelector('#header-dropdowns .select-row');
                    const maybeVM = selectRowInner && selectRowInner.querySelector('.view-katalog-btn');
                ...
                // in the header (per request). Change the header add button label to "Lihat Katalog".
                // ensure katalog header is removed when showing Unit so the "Katalog Unit"
                // title/logo doesn't persist after returning from Katalog view
                try { const mhTop = document.querySelector('#header-dropdowns .select-row .katalog-header'); if (mhTop && mhTop.parentNode) mhTop.parentNode.removeChild(mhTop); } catch (e) { }
                if (kategoriWrap) kategoriWrap.style.display = 'none';
                if (jenisWrap) jenisWrap.style.display = 'none';
                // hide stok filter entirely for Unit page
                if (stokWrap) stokWrap.style.display = 'none';
                // Update header Add button to read "Lihat Katalog" when on Unit page
                try {
                    ...
                    // Create or show a dedicated 'Lihat Katalog' button in header (.view-katalog-btn)
                    const selectRow = document.querySelector('#header-dropdowns .select-row');
                    if (selectRow) {
                        let vm = selectRow.querySelector('.view-katalog-btn');
                        if (!vm) {
                            vm = document.createElement('button');
                            vm.className = 'view-katalog-btn';
                            vm.setAttribute('aria-label', 'Lihat Katalog');
                            vm.textContent = 'Lihat Katalog';
                            // insert left of filters
                            selectRow.insertBefore(vm, selectRow.firstChild);
                        } else {
                            vm.style.display = '';
                            // ensure any header back button is hidden when showing Lihat Katalog
                        ...
                        const handler = async (ev) => {
                            ...
                            try {
                                ...
                                if (window.appKatalogUI && typeof window.appKatalogUI.renderKatalogPage === 'function') await window.appKatalogUI.renderKatalogPage();
                                try { if (window.appNavigation && typeof window.appNavigation.updateHeaderSearchVisibility === 'function') window.appNavigation.updateHeaderSearchVisibility('katalog'); } catch (e) { }
                            } catch (e) { /* ignore */ }
                        };
                        vm.addEventListener('click', async (ev) => {
                            ...
                            await handler(ev);
                            try {
                                // hide the Lihat Katalog button while inside Katalog
                            ...
                            try {
                                ...
                                if (selectRowInner) {
                                    ...
                                    const navToUnit = function (ev2) {
                                        ...
                                        // after navigation, hide back button and restore Lihat Katalog
                                        try {
                        ...
                        // ensure katalog header (title/logo) is hidden on Unit view
                        try { const mh = document.querySelector('#header-dropdowns .select-row .katalog-header'); if (mh) mh.style.display = 'none'; } catch (e) { }
                    }
            ...
            } else if (activeSectionId === 'katalog') {
                // For Katalog page: hide stok filter and status filter in header.
                if (stokWrap) stokWrap.style.display = 'none';
                const statusWrap = document.querySelector('#resetStatus') && document.querySelector('#resetStatus').closest('.filter-wrap');
                if (statusWrap) statusWrap.style.display = 'none';
                ...
                try {
                    const addBtn = document.querySelector('.add-btn');
                    if (addBtn) {
                        // hide add button entirely in Katalog view to avoid conflicting actions
                        try {
                            if (addBtn.__navBackHandler) { try { addBtn.removeEventListener('click', addBtn.__navBackHandler); } catch (e) { } delete addBtn.__navBackHandler; }
                ...
                // ensure katalog header (title/logo) is visible on Katalog view
                try { const mh = document.querySelector('#header-dropdowns .select-row .katalog-header'); if (mh) mh.style.display = ''; } catch (e) { }
            } else {
                // restore defaults for non-jasa pages
                // Ensure the 'Lihat Katalog' header control is not present on Item (and other non-unit pages)
                try {
                    const selectRowInner = document.querySelector('#header-dropdowns .select-row');
                    const maybeVM = selectRowInner && selectRowInner.querySelector('.view-katalog-btn');
                    if (maybeVM && maybeVM.parentNode) maybeVM.parentNode.removeChild(maybeVM);
        ...
        try {
            ...
            if (addBtn) {
                ...
                // If we're in Katalog mode, show a clear Back button that returns to Unit.
                if (activeSectionId === 'katalog') {
                    ...
                    // For Unit page we want the add button to be a plain text "Lihat Katalog"
                    try {
                        addBtn.innerHTML = 'Lihat Katalog';
                        addBtn.setAttribute('aria-label', 'Lihat Katalog');
                    } catch (e) { /* ignore */ }
                    // Reattach items handler if available so clicking Lihat Katalog still runs itemsUI handler
                    try {
                    ...
                    try {
                        ...
                        else if (initialAddIconSrc) addBtn.innerHTML = `<img src="${initialAddIconSrc}" alt="Lihat Katalog"/> ${initialAddText || 'Lihat Katalog'}`;
