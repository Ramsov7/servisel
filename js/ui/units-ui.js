...
(function () {
    ...
    async function renderList() {
        ...
        try {
            ...
            try {
                // New schema: use `katalog` (replaces old `master_unit`)
                const probeMaster = await safeSelect('katalog', 'id_katalog,nama_brand,nama_model,gambar_url,spesifikasi', 500);
            ...
            // prefer katalog (replacement for master_unit) but keep master_unit as fallback
            const tableCandidates = ['unit', 'katalog', 'master_unit'];
            let data = null;
            ...
            if (masterUnits && masterUnits.length) {
                try {
                    const idKatalogs = masterUnits.map(m => (m.id_katalog || m.id_master)).filter(Boolean);
                    const counts = {};
                    if (idKatalogs.length) {
                        try {
                            // Use `in` for matching multiple id_katalog values. Request only
                            // columns we know exist in the current schema (id_katalog,status).
                            // This avoids asking for id_master which may not be present and
                            // avoids building a very long OR query string.
                            const unitsRes = await sup.from('unit').select('id_katalog,status').in('id_katalog', idKatalogs).limit(2000);
                            const unitsRows = (unitsRes && unitsRes.data) ? unitsRes.data : [];
                            if (unitsRows && unitsRows.length) {
                                unitsRows.forEach(r => {
                                    ...
                                    const key = r.id_katalog || null;
                                    if (inStock && key) counts[key] = (counts[key] || 0) + 1;
                                });
                            }
                        } catch (e) {
                    ...
                    const availableModels = (masterUnits || []).filter(m => {
                        const key = (m.id_katalog || m.id_master || null);
                    ...
                    if ((availableModels || []).length) {
                        availableModels.forEach(m => {
                            ...
                            const stockNum = counts[(m.id_katalog || m.id_master || null)] || 0;
                            mcard.innerHTML = `
                                <div class="card-main">
                    ...
                    try {
                        const { data: unlinkedUnits } = await sup.from('unit').select('*').is('id_katalog', null).eq('status', 'aktif').limit(200);
                            ...
                            if (!imgSrc) {
                                ...
                                if (kid) {
                                    if (katalogUnits && katalogUnits.length) {
                                        ...
                                        if (k && (k.gambar_url || k.gambar_master)) imgSrc = (k.gambar_url || k.gambar_master || '').toString().trim();
                                    }
                                    if (!imgSrc && masterUnits && masterUnits.length) {
                                        const m = masterUnits.find(x => (x.id_master || x.id_katalog || '').toString() === (kid || '').toString());
                                        if (m && (m.gambar_url || m.gambar_master)) imgSrc = (m.gambar_url || m.gambar_master || '').toString().trim();
                                    }
                                }
                            }
                        } catch (e) {
    ...
    async function insertUnit(payload) {
        ...
        try {
            if (resolvedTable === 'unit' && ((!insertObj.id_katalog || insertObj.id_katalog === null) && (!insertObj.id_master || insertObj.id_master === null))) {
                const tryName = insertObj.nama_unit || payload.nama_unit || '';
                const tryCode = insertObj.kode_unit || payload.kode_unit || '';
                if (tryName || tryCode) {
                    try {
                        let q = sup.from('katalog').select('id_katalog,nama_model,nama_brand').limit(5);
                        ...
                        if (probe && !probe.error && probe.data && probe.data.length) {
                            ...
                            if (cand && (cand.id_katalog || cand.id_master)) insertObj.id_katalog = (cand.id_katalog || cand.id_master);
                        }
                    } catch (e) { }
                }
            }
            ...
            if (resolvedTable === 'unit' && ((!insertObj.id_katalog || insertObj.id_katalog === null) && (!insertObj.id_master || insertObj.id_master === null))) {
                ...
                if (idField && created && (tryName || tryCode)) {
                    try {
                        ...
                        if (probe && !probe.error && probe.data && probe.data.length) {
                            const cand = probe.data[0];
                            if (cand && (cand.id_katalog || cand.id_master)) {
                                const upd = {};
                                upd['id_katalog'] = (cand.id_katalog || cand.id_master);
                                try {
                                    await sup.from(resolvedTable).update(upd).eq(idField, created[idField]);
                                    // normalize created record to include id_katalog for renderer
                                    created.id_katalog = (cand.id_katalog || cand.id_master);
                                    console.debug('unitsUI.insertUnit: linked inserted unit to katalog', (cand.id_katalog || cand.id_master), created[idField]);
                                } catch (e) { }
                            }
                        }
                    } catch (e) { }
        ...
        try {
            ...
            for (const t of tableCandidates) {
                try {
                    ...
                    const rows = data || [];
                    existing = (rows || []).map(r => ({ kode_unit: r.kode_unit || r.kode_model || r.id_katalog || r.id_master || '' }));
                    if (container) container.dataset.unitsTable = t;
                    break;
                } catch (e) {
                    lastErr = e;
