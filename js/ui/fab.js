...
(function () {
    ...
    // Create a modal that only has a large X close button (no Save/Batal actions).
    function createCloseOnlyModal(title, innerHTML) {
        const modal = document.createElement('div');
        modal.className = 'simple-modal modal';
        modal.setAttribute('aria-hidden', 'false');
        modal.innerHTML = '\n            <div class="modal-backdrop" data-action="close"></div>\n            <div class="modal-window">\n                <div class="modal-header"><h3>' + escapeHtml(title) + '</h3><button class="modal-close large" aria-label="Tutup">×</button></div>\n                <div class="modal-body">' + innerHTML + '</div>\n            </div>\n        ';
        document.body.appendChild(modal);
        try { document.body.classList.add('modal-open'); } catch (e) { }
        // Only the X closes the modal; clicking backdrop won't close (user chooses X explicitly)
        const closeBtn = modal.querySelector('.modal-close'); if (closeBtn) closeBtn.addEventListener('click', () => closeModalByEl(modal));
        return modal;
    }
    ...
    function openUnitPurchaseForm() {
        try {
            ...
            // Insert a clear (x) button overlayed inside tipe_unit input and wire behavior.
            try {
                const tipeInput = section.querySelector('input[name="tipe_unit"]');
                if (tipeInput) {
                    // create clear button only if not already present
                    if (!section.querySelector('#clear-tipe-unit-btn')) {
                        // wrap input in a relative container so we can absolutely position the clear button
                        const wrap = document.createElement('div');
                        wrap.style.position = 'relative';
                        wrap.style.display = 'flex';
                        wrap.style.alignItems = 'center';
                        wrap.style.width = '100%';
                        // replace tipeInput with wrap then append input into wrap
                        const parent = tipeInput.parentNode;
                        if (parent) {
                            parent.replaceChild(wrap, tipeInput);
                            wrap.appendChild(tipeInput);
                        }
                        // adjust input padding so text doesn't go under the button
                        try { tipeInput.style.paddingRight = '34px'; } catch (e) { }

                        const clearBtn = document.createElement('button');
                        clearBtn.type = 'button';
                        clearBtn.id = 'clear-tipe-unit-btn';
                        clearBtn.setAttribute('aria-label', 'Hapus teks');
                        clearBtn.title = 'Hapus';
                        clearBtn.textContent = '×';
                        // style as small overlay inside input
                        ...
                        // hidden by default; shown only after user selects a type
                        ...
                        // Clear button behavior: clear input, remove readonly, focus
                        clearBtn.addEventListener('click', (ev) => {
                            ev.preventDefault();
                            try {
                                tipeInput.value = '';
                                tipeInput.readOnly = false;
                                // remove marker class if any
                                wrap.classList.remove('tipe-selected');
                                // hide the clear button until another selection is made
            ...
            try {
                ...
                if (cariBtn) {
                    cariBtn.addEventListener('click', async (ev) => {
                        ev.preventDefault();
                        const input = section.querySelector('input[name="tipe_unit"]');
                        const q = input ? (input.value || '').trim() : '';
                        if (!q) { alert('Masukkan kata kunci pencarian tipe unit.'); return; }
                        try {
                            const results = await searchKatalog(q, 60);
                            const modal = createResultsModal('Hasil Pencarian: ' + q, results, (item) => {
                                try {
                                    if (input) {
                                        input.value = (item.nama_model || item.kode_model || '');
                                        // make input readonly so user cannot manually delete the text
                                        input.readOnly = true;
                                        // mark wrapper if present to allow visual state
                                        const wrap = input.parentNode;
                                        if (wrap) {
                                            wrap.classList.add('tipe-selected');
                                            // show the clear (×) button that was injected into the wrapper
                                            const clearBtn = wrap.querySelector('#clear-tipe-unit-btn');
                                            try { if (clearBtn) clearBtn.style.display = 'inline-flex'; } catch (e) { }
                                        }
                                    }
                                } catch (e) { }
                            });
                        } catch (e) {
                            console.error('Pencarian katalog gagal', e);
                            alert('Terjadi kesalahan saat mencari katalog. Lihat console.');
                        }
                    });
                }
            } catch (e) { }
    ...
    // Search katalog table (or master_unit) for matching model names.
    async function searchKatalog(q, limit = 50) {
        try {
            const supabaseClient = (typeof getSupabase === 'function') ? getSupabase() : (window.supabase || null);
            if (supabaseClient && typeof supabaseClient.from === 'function') {
                // use Supabase client to search katalog
                const filter = `%${q.replace(/%/g, '')}%`;
                const { data, error } = await supabaseClient
                    .from('katalog')
                    .select('id_katalog,nama_brand,nama_model,gambar_url,spesifikasi')
                    .or(`nama_model.ilike.${filter},nama_brand.ilike.${filter}`)
                    .limit(limit);
                if (error) throw error;
                return (data || []).map(d => ({ id_katalog: d.id_katalog, nama_brand: d.nama_brand, nama_model: d.nama_model, gambar_url: d.gambar_url, spesifikasi: d.spesifikasi }));
            }

            // fallback: try an in-memory cache if available
            const cache = window.katalogCache || window._katalogCache || [];
            const ql = q.toLowerCase();
            return cache.filter(k => (String(k.nama_model || '') + ' ' + String(k.nama_brand || '')).toLowerCase().includes(ql)).slice(0, limit);
        } catch (e) {
            console.error('searchKatalog failed', e);
            return [];
        }
    }

    // Create a modal listing results (items) and call onSelect(item) when an item is clicked.
    function createResultsModal(title, items, onSelect) {
        const modal = document.createElement('div');
        modal.className = 'simple-modal modal';
        modal.setAttribute('aria-hidden', 'false');
        ...
        if (!Array.isArray(items) || items.length === 0) {
            bodyHtml = '<div class="results-empty">Tidak ada hasil</div>';
        } else {
            bodyHtml = '<div class="results-list">';
            items.forEach((it, idx) => {
                const img = it.gambar_url ? '<div class="result-thumb"><img src="' + escapeHtml(it.gambar_url) + '" alt="' + escapeHtml(it.nama_model || '') + '"></div>' : '';
                const title = '<div class="result-title"><strong>' + escapeHtml(it.nama_model || '') + '</strong><div class="result-sub">' + escapeHtml(it.nama_brand || '') + '</div></div>';
                bodyHtml += '<button type="button" class="result-item" data-idx="' + idx + '">' + img + title + '</button>';
            });
            bodyHtml += '</div>';
        }
        ...
        try { document.body.classList.add('modal-open'); } catch (e) { }
        // wire close handlers
        const backdrop = modal.querySelector('.modal-backdrop'); if (backdrop) backdrop.addEventListener('click', () => closeModalByEl(modal));
        const closeBtn = modal.querySelector('.modal-close'); if (closeBtn) closeBtn.addEventListener('click', () => closeModalByEl(modal));

        // wire item clicks
        Array.from(modal.querySelectorAll('.result-item')).forEach((el) => {
            el.addEventListener('click', (ev) => {
                ev.preventDefault();
                const idx = Number(el.getAttribute('data-idx'));
                const item = items[idx];
                try { if (typeof onSelect === 'function') onSelect(item); } catch (e) { console.error(e); }
