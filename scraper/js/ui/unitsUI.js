/* unitsUI.js — compatibility wrapper (loads canonical kebab-case file)
   Keeps older references working while canonical file is `units-ui.js`. */

(function () {
    if (typeof window === 'undefined') return;
    // If canonical implementation already present, do nothing
    if (window.appUnitsUI) return;
    try {
        var s = document.createElement('script');
        s.src = 'js/ui/units-ui.js';
...
(function () {
    ...
    async function renderList() {
        ...
        try {
            ...
            try {
                // Use only columns that exist in your master_unit schema to avoid 400 errors
            ...
            try {
                // Newer schema may provide a `katalog` table with katalog.gambar_url
            ...
            if (masterUnits && masterUnits.length) {
                ...
                try {
                    ...
                    // Ensure brand-level cards for Motorola and Asus are present.
                    // Compute brand stock by summing counts for master entries of that brand
                    try {
                        const ensureBrands = ['Motorola', 'Asus'];
                        const existingTitles = new Set(Array.from(modelsGrid.querySelectorAll('.card-title')).map(el => (el.textContent || '').trim().toLowerCase()));
                        ensureBrands.forEach(b => {
                            const key = (b || '').toString().toLowerCase();
                            // Avoid adding duplicate brand cards if a model with the same visible title exists
                            if (existingTitles.has(key)) return;
                            // Sum stock for master models that match this brand (case-insensitive)
                            let brandStock = 0;
                            try {
                                (masterUnits || []).forEach(mu => {
                                    if ((mu.nama_brand || mu.brand || '').toString().toLowerCase() === key) {
                                        brandStock += (counts[mu.id_master] || 0);
                                    }
                                });
                            } catch (e) { brandStock = 0; }

                            const bcard = document.createElement('div');
                            // Use the same classes as model cards so behavior/styles match exactly
                            bcard.className = 'item-card compact modern model-card';
                            bcard.innerHTML = `
                                <div class="card-main">
                                    <div class="img-wrap"></div>
                                    <div class="card-meta">
                                        <div class="card-title">${escapeHtml(b)}</div>
                                        <div class="card-stock">Stok: <span class="stock-value">${brandStock || 0}</span></div>
                                    </div>
                                </div>
                            `;
                            try { insertBrandLogo(bcard.querySelector('.img-wrap'), b); } catch (e) { }
                            // Append like other model cards so behavior is identical
                            modelsGrid.appendChild(bcard);
                        });
                    } catch (e) { /* ignore brand injection errors */ }
                    ...
                    // Build a brands container (mirror masterUI structure) so brand cards
                    // appear in the same `.brand-cards.master-brands.items-grid` element.
                    try {
                        ...
                        developerBrands.forEach(db => {
                            const key = (db || '').toString().toLowerCase();
                            let brandCount = 0;
                            try {
                                (masterUnits || []).forEach(mu => {
                                    const bname = (mu.nama_brand || mu.brand || '').toString().toLowerCase();
                                    if (bname === key) brandCount += (counts[mu.id_master] || 0);
                                });
                            } catch (e) { brandCount = 0; }
                            const cardB = document.createElement('div');
                            cardB.className = 'item-card compact modern brand-card master-brand';
                            cardB.dataset.brand = key;
                            cardB.innerHTML = `
                                <div class="card-main">
                                    <div class="img-wrap"></div>
                                    <div class="card-meta">
                                        <div class="card-title">${escapeHtml(db)}</div>
                                        <div class="card-stock">${brandCount} tipe</div>
                                    </div>
                                </div>
                            `;
                            try { insertBrandLogo(cardB.querySelector('.img-wrap'), db); } catch (e) { }
                            brandWrap.appendChild(cardB);
                        });
                        // put brandWrap before modelsGrid in the card body
                        cb.appendChild(brandWrap);
                    } catch (e) { /* ignore brand container errors */ }
            ...
            if (!data || data.length === 0) {
                ...
                // Batch-fetch uploaded photos (unit_foto) for these units so uploaded images are preferred
                let unitFotoMap = {};
                try {
                ...
                // Create a brand-cards container to match master UI layout
                try {
                    const brandWrap = document.createElement('div');
                    brandWrap.className = 'brand-cards master-brands items-grid';
                    const developerBrands = ['Apple', 'Huawei', 'Infinix', 'Itel', 'Oppo', 'Realme', 'Samsung', 'Tecno', 'Vivo', 'Xiaomi', 'Motorola', 'Asus'];
                    developerBrands.forEach(db => {
                        ...
                        try {
                            (filtered || []).forEach(u => {
                                const bname = (u.nama_brand || guessBrand(u.nama_unit) || '').toString().toLowerCase();
                                if (bname === key) brandStock += Number(u.stok ?? u.stok_item ?? u.stock ?? 0) || 0;
                            });
                        } catch (e) { brandStock = 0; }
                        const cardB = document.createElement('div');
                        cardB.className = 'item-card compact modern brand-card master-brand';
                        ...
                        cardB.innerHTML = `
                            <div class="card-main">
                                ...
                                <div class="card-meta">
                                    <div class="card-title">${escapeHtml(db)}</div>
                                    <div class="card-stock">${brandStock} tipe</div>
                                </div>
                            </div>
                        `;
                        try { insertBrandLogo(cardB.querySelector('.img-wrap'), db); } catch (e) { }
                    ...
                    // append brandWrap before unitGrid so DOM matches master card layout
                    listWrap.appendChild(brandWrap);
                } catch (e) { /* ignore */ }

                // Add brand-level cards (Motorola, Asus) even when masterUnits isn't present.
                try {
                    const ensureBrands = ['Motorola', 'Asus'];
                    // compute stock for each brand from the filtered unit list
                    ensureBrands.forEach(b => {
                        const key = (b || '').toString().toLowerCase();
                        let brandStock = 0;
                        try {
                            (filtered || []).forEach(u => {
                                const bname = (u.nama_brand || guessBrand(u.nama_unit) || '').toString().toLowerCase();
                                if (bname === key) {
                                    brandStock += Number(u.stok ?? u.stok_item ?? u.stock ?? 0) || 0;
                                }
                            });
                        } catch (e) { brandStock = 0; }
                        ...
                        `;
                        try { insertBrandLogo(bcard.querySelector('.img-wrap'), b); } catch (e) { }
                        // insert at the start so brand cards appear before unit items
                        if (unitGrid.firstChild) unitGrid.insertBefore(bcard, unitGrid.firstChild);
                        else unitGrid.appendChild(bcard);
                    });
                } catch (e) { /* ignore */ }
                ...
                filtered.forEach(u => {
                    ...
                    if (imgWrap) {
                        ...
                        // Prefer uploaded unit_foto -> foto_sesudah/foto_sebelum -> image_url/foto -> katalog.gambar_url -> master_unit.gambar_master
                        let imgSrc = '';
                        try {
